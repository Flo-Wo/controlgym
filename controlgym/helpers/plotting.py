...
def _plot_pde(
    ...
    contour_cmap_dict = {
        "allen_cahn": "Spectral_r",
        "burgers": "coolwarm",
        "kuramoto_sivashinsky": "viridis",
        "cahn_hilliard": "Spectral_r",
        "convection_diffusion_reaction": "coolwarm",
        "fisher": "Spectral_r",
        "ginzburg_landau": "Spectral_r",
        "korteweg_de_vries": "Spectral_r",
    }
    sparcity_constant_dict = {
        "allen_cahn": 10,
        "burgers": 5,
        "kuramoto_sivashinsky": 50,
        "cahn_hilliard": 5,
        "convection_diffusion_reaction": 5,
        "fisher": 10,
        "ginzburg_landau": 10,
        "korteweg_de_vries": 5,
    }
    if contour:
        ...
        cmap2 = plt.get_cmap("jet")(
            np.linspace(0, 1, int(env.n_steps / sparcity_constant_dict[env.id]) + 1)
        )
        for t in range(0, env.n_steps, sparcity_constant_dict[env.id]):
            ax2.plot(
                env.domain_coordinates,
                env.state_traj[:, t],
                color=cmap2[int(t / sparcity_constant_dict[env.id])],
                alpha=0.8,
            )
    ...
    if surface3d:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(save_dir, "surface3d.png"), dpi=dpi, bbox_inches="tight"
        )
        if display:
            plt.show()
        plt.close(fig)
...
def _plot_coupled_pde(
    ...
    if contour:
        ...
        cf1 = ax1.contourf(
            time,
            env.domain_coordinates,
            env.state_traj[: env.n_state_half, :],
            levels=50,
            extend="both",
            cmap=plt.get_cmap(contour_cmap_dict[env.id][0]),
        )
        fig.colorbar(
            cf1, orientation="vertical", label="state (first half)", format="%.2f"
        )
        ax1.set_xlabel("simulation time")
        ax1.set_ylabel("domain coordinates")
        ...
        for t in range(0, env.n_steps, sparcity_constant_dict[env.id]):
            ax2.plot(
                env.domain_coordinates,
                env.state_traj[: env.n_state_half, t],
                color=cmap2[int(t / sparcity_constant_dict[env.id])],
                alpha=0.8,
            )
            ax4.plot(
        ...
        fig.colorbar(
            cf3, orientation="vertical", label="state (second half)", format="%.2f"
        )
...
def _plot_linear(env, save_dir: str = None, display: bool = False, dpi: int = 100):
    ...
    if env.n_state <= 6:
        ax.legend(
            [rf"$s_{i}$" for i in range(env.n_state)], loc="upper right", fontsize=13
        )
    plt.tick_params(
        axis="both", which="major", labelsize=13
    )  # Set font size for major ticks
    plt.tick_params(
        axis="both", which="minor", labelsize=13
    )  # Set font size for minor ticks
...
def _plot_linearPDE_eigen(
    env, save_dir: str = None, display: bool = False, dpi: int = 100
):
    ...
    ax.scatter(
        X, Y, color="tab:orange", marker="*", s=14, label="Numerical Eigenvalues"
    )
