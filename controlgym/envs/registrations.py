from controlgym.envs import (
    LinearControlEnv,
    AllenCahnEnv,
    BurgersEnv,
    CahnHilliardEnv,
    ConvectionDiffusionReactionEnv,
    FisherEnv,
    GinzburgLandauEnv,
    KortewegDeVriesEnv,
    KuramotoSivashinskyEnv,
    SchrodingerEnv,
    WaveEnv,
)
...
def make(id: str, **kwargs):
    ...
    if id in linear_control_id_list:
        ...
        raise ValueError(
            "Invalid environment id. " + "Valid environment ids include: ",
            linear_control_id_list + pde_id_list,
        )
...
def _to_camel_env_id(id: str):
    ...
    if len(id) == 0:
        ...
        return (
            camel_id[0].capitalize()
            + "".join(i.capitalize() for i in camel_id[1:])
            + "Env"
        )
